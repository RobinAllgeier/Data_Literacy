...
All analyses can be reproduced using the accompanying code repository.\footnote{\url{https://github.com/RobinAllgeier/Data_Literacy}}
...
We gathered domain knowledge through a personal interview with a staff member of the Tübingen City Library, which provided contextual insights into data collection procedures, internal system conventions, and data quality practices.
...
\begin{figure}[ht]
    ...
    \caption{
        ...
        The orange line indicates the share of loans returned late, and the red line the share of transactions removed during data cleaning (right axis).
    }
...
We then verified temporal consistency and plausibility by checking that return dates never preceded borrowing dates, that derived loan durations and days-late values were non-negative, and that almost all loans complied with the library's maximum borrowing period.
Under library rules, users may borrow items for 28 days with up to six extensions of 28 days each, yielding a maximum loan period of 196 days.
...
\begin{itemize}
    \item Removal of transactions with missing return timestamps, which represent items still on loan at the time of data export and cannot be used for analyses of completed borrowing cycles. This primarily affected 2025 data (nearly 50\,000 transactions still active), while 2024 had only about 90 such cases.
...
We ordered sessions chronologically per user and assigned a session index as a proxy for accumulating experience.
We derived session-level indicators via aggregation.
...
We excluded sessions with ties from this analysis.
