...
\textbf{Late Return Consistency:} The consistency between late return flags and the number of days late was validated. Additionally, implausible values in the extensions column were checked, only a few cases with more than six extentions were identified (0.003\% of rows), which  but were not further investugated due to their negligible impact.
\textbf{Dublicate Analysis:} Various forms of duplicates were examined. No exact duplicates were found, but identical borrowing timestamps occurred when useres borrowed multiple items in one transaction, limited to the same user ID and a maximum of seven items per transaction. These were retained as they represent valid borrowing behavior.
...
\begin{itemize}
    ...
    \item Borrowings taken by library`s staff members and system entries, which do not reflect typical user behavior, were removed based on their user category.
