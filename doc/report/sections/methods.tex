...
Although such a test is expected to yield low p-values due to human time structures and opening hours, as hardly any user distributes their visits stochastically over the entire week, the comparison of the distributions (for users with $N \geq 10$ visits) provides valuable insights. 
The observed distribution of entropy is significantly shifted to the left compared to the randomized baseline. 
This not only confirms the existence of general routines, but also identifies a significant group of users whose entropy is below the 1\% percentile of the random distribution. 
A generalizable statement about the entire user base must be made with caution due to the structural bias of the data set.
However, it appears that a certain group of “habitual users” can be isolated.
